...
class FakeDao : HealthOsDao {
    ...
    override suspend fun measurements(metric: String, limit: Int): List<MeasurementEntity> =
        measurementsList.filter { it.metricType.name.equals(metric, ignoreCase = true) }.take(limit)
    ...
    override suspend fun updateSyncStatus(ids: List<String>, status: SyncStatus) {
        updatedIds = ids
...
class FakePatientApi : PatientApiService {
    ...
    override suspend fun syncMeasurements(request: SyncMeasurementsRequestDto): Response<ApiResponse<SyncMeasurementsResponseDto>> {
        if (shouldFail) throw RuntimeException("Network timeout")
        syncedRequest = request
        val payload = SyncMeasurementsResponseDto(insertedCount = request.measurements.size)
        return Response.success(ApiResponse(status = "success", data = payload))
    }

    override suspend fun getMeasurements(patientId: String, type: String?, days: Int?, limit: Int?): Response<ApiResponse<List<MeasurementDto>>> =
    ...
    override suspend fun addMedication(patientId: String, request: CreateMedicationRequestDto): Response<ApiResponse<MedicationDto>> =
        Response.success(ApiResponse(status = "success", data = MedicationDto(id = "med_1", name = request.name, dosage = request.dosage, schedule = request.schedule)))

    override suspend fun logMedication(patientId: String, request: MedicationLogRequestDto): Response<ApiResponse<Map<String, Any>>> =
        Response.success(ApiResponse(status = "success", data = emptyMap()))

    override suspend fun getAlertDetail(alertId: String): Response<ApiResponse<AlertDto>> =
        Response.success(ApiResponse(status = "success", data = AlertDto(id = alertId)))

    override suspend fun acknowledgeAlert(alertId: String): Response<ApiResponse<AlertDto>> =
        Response.success(ApiResponse(status = "success", data = AlertDto(id = alertId, acknowledged = true)))
...
class SyncWorkersTest {
    ...
    @Test
    fun testPendingMeasurementsProcessing() = runTest {
        val fakeDao = FakeDao()
        ...
        val dtos = pending.map {
            com.healthos.data.remote.SyncMeasurementItemDto(
        ...
        val response = fakeApi.syncMeasurements(SyncMeasurementsRequestDto(measurements = dtos))
        assertTrue(response.isSuccessful)
        assertEquals(2, response.body()?.data?.insertedCount)
    ...
    @Test
    fun testSyncFailsGracefullyOnNetworkError() = runTest {
        val fakeApi = FakePatientApi().apply { shouldFail = true }

        var failed = false
        try {
            fakeApi.syncMeasurements(SyncMeasurementsRequestDto(measurements = emptyList()))
        } catch (_: Exception) {
            failed = true
        }
        assertTrue(failed)
